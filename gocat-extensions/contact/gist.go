...
func (g GIST) UploadFileBytes(profile map[string]interface{}, uploadName string, data []byte, link_id string) error {
	...
	for i := 0; i < numChunks; i++ {
		...
		gistDescription := getGistDescriptionForUpload(uploadId, link_id, encodedFilename, i+1, numChunks)
...
func getGistDescriptionForUpload(uploadId string, link_id string, encodedFilename string, chunkNum int, totalChunks int) string {
	return fmt.Sprintf("upload:%s:%s:%s:%d:%d", uploadId, link_id, encodedFilename, chunkNum, totalChunks)
}
